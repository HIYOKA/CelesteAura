...
const ReviewGraphs = () => {
  ...
  const data = [
    {
      ...
      avgRating:
        TrousersreviewCount === 0
          ...
          : Number((TrousersratingSum / TrousersreviewCount).toFixed(1)), // toFixed() : 소수점 자리수를 지정
    },
    {
      category: "Top",
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const productIds = result.data.map((review) => review.product_id); // map : 배열의 각 요소에 대해 주어진 함수를 호출한 결과를 모아 새로운 배열을 반환
